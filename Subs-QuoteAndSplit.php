...
function qas_add_modsettings (&$config_vars)
{
	$config_vars[] = array('check', 'qas_post_split_notification');
	$config_vars[] = array('large_text', 'qas_default_notify_message');
	$config_vars[] = array('large_text', 'qas_default_new_message');
}
...
function qas_PostMovedToTopic ($old_topic_id, $new_topic_id)
{
	global $smcFunc, $modSettings, $context, $board_info, $board, $scripturl, $txt, $webmaster_email, $user_info;

	$old_topic_id = (int) $old_topic_id;

	if (empty($modSettings['qas_post_split_notification']) || empty($old_topic_id) || empty($new_topic_id))
		return false;

	$topics = qas_getOriginalTopicInfo(array($old_topic_id, $new_topic_id));

	if (!empty($modSettings['qas_default_notify_message']))
	{
		$message = str_replace(
			array(
				'{NEXT_TOPIC_URL}',
			...
			$modSettings['qas_default_notify_message']
		);
	...
	if (!empty($modSettings['qas_default_new_message']))
	{
		$message = str_replace(
			array(
				'{PREV_TOPIC_URL}',
			...
			$modSettings['qas_default_new_message']
		);
...
function qas_getOriginalTopicInfo ($topics_id = null)
{
